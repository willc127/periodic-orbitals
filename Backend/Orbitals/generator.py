...
def orbitals_generator(
    ...
    if (n, l, m) in PROBLEMATIC_ORBITALS_XY_OFFSETS and projection == "xy":
        samples = max(samples, 600)  # Aumentar para 600 se for problemático

    # Se offset é 0.0 e é um orbital problemático, usar offset automático
    # Orbitais com m ≠ 0 ou l ≥ 3 tendem a ter valores nulos em z=0 no plano XY
    if plano_offset == 0.0 and projection == "xy":
        ...
        # Padrão: todos com m ≠ 0 precisam de offset
            ...
            # Quanto maior |m|, l, e n, maior o offset necessário
            # Fórmula melhorada com coeficientes maiores para casos extremos
    ...
    try:
        ...
        if "vmin must be less or equal to vmax" in error_msg or plano_offset < 1.5:
            # Tentar offsets cada vez maiores até valores extremos
            fallback_offsets = [
                0.05,
                0.1,
                0.15,
